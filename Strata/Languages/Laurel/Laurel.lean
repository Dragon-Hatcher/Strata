...
`SubscriptElim` and downstream passes reference these names when lowering
`Seq<T>` and `Array<T>` to Core. They are collected here so changes do not
silently diverge between passes.
-/

/-- Namespace prefix shared by all sequence operations. -/
...
/-- `Sequence.append(s1, s2) : Seq<T>` — concatenate two sequences. -/
