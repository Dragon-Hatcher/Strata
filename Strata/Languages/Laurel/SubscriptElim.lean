...
def elimExpr (model : SemanticModel) (expr : StmtExprMd) : StmtExprMd :=
  ...
  | .Subscript target index (some value) =>
    ...
    if isArrayType model target then
      -- `a[i := v]` on Array<T>: not supported (validator reports).
      -- Desugar the read-side to keep downstream typecheck simple; the
      -- validator will have already surfaced the diagnostic in commit 2.
  ...
  | .Assign [⟨.Subscript target index none, ssrc⟩] value =>
    ...
    -- On Seq<T> this is a user error; `validateSubscriptUsage` (Layer 1 diagnostics)
    -- reports the helpful message. We replace the statement with an empty block so
    -- downstream passes don't additionally complain about the now-ill-shaped assign.
    let target' := elimExpr model target
    let index' := elimExpr model index
    let value' := elimExpr model value
    if isArrayType model target then
      ...
      -- No-op placeholder; the validator has already emitted the real error.
      -- We still recurse into sub-expressions so that nested Subscripts/Array.length
      -- are desugared (harmless here, but keeps the AST consistent).
      let _ := (target', index', value')
      ⟨.Block [] none, src⟩
...
/-- The synthetic `$Array` composite, containing a single `$data: Seq<int>`
    field. The element type is erased (Core's polymorphic type inference
    handles the actual element types). -/
