...
Currently only `Array<int>` is fully supported at the SMT level; other element types for
arrays may fail to verify. `Seq<T>` supports any element type.
...
`$data: Seq<T>` field. The `$` prefix is a naming convention used for compiler-internal
names to avoid collisions with user-declared types. The `$Array` composite is only
injected into programs that actually use `Array<T>`.
